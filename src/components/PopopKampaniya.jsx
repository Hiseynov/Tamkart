          ...
          <div className="popop-button">
            ...
            <div className="popop-word">
              ...
              <div className="popop-categories">
                {def.fistP ? (
                  <>
                    <p className="linearHeagh">{def.fistP[0]}</p>
                  </>
                ) : (
                  ""
                )}
                {def.info ? (
                  <>
                    <p>
                      ...
                      {def.link ? (
                        <a href={def.link[1]}>{def.link[0]}</a>
                      ) : (
                        ""
                      )}{" "}
                      {def.b ? <b>{def.b[0]}</b> : ""}
                      ...
                      {def.partnorSiyahi ? (
                        <>
                          <span>{def.partnorSiyahi}</span>
                          <a href={def.partnorLink[1]}>{def.partnorLink[0]}</a>
                        </>
                      ) : (
                        ""
                      )}
                      {def.link ? <a href={def.link[2]}>{def.link[3]}</a> : ""}
            ...
            {def.button ? (
              <div className="popop-button-link">
            ...
            {def.buttonLink ? (
              <div className="popop-button-link">
                <Link to={"/Login-Registr"}>{def.buttonLink}</Link>
              </div>
            ) : (
              ""
            )}
            ...
            )}
            {
              def.buttonLogin &&(
                <div className="popop-button-link">
                   <Link to={localStorage.getItem('user')? "/Kartlar":"/login"}>{def.buttonLogin}</Link>
                </div>
               )
            }
            {def.second ? (
            ...
            {def.button2 ? 
            <div className="popop-button-link">
               <a href={def.button2[1]}>{def.button2[0]}</a> 
            </div>: ""}
          </div>
        </div>
      </div>
